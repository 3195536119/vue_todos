...
//删除todo，暂时未完善
Mock.mock(/deleteTodo/, 'get', (param) => { //三个参数。第一个：路径，第二个：请求方式post/get，第三个：回调，返回值
    console.log(`deleteTodo,params:`, param)
    let url = param.url
    console.log(url)
    console.log(getParams(url))
    let paramObj = getParams(url);
...
//添加一个小代办id,text
Mock.mock(/addItem/, 'get', (param) => { //三个参数。第一个：路径，第二个：请求方式post/get，第三个：回调，返回值
    console.log(`addItem,params:`, param)
    let url = param.url
    console.log(url)
    console.log(getParams(url))
    let paramObj = getParams(url);
    data.some(item => {
        if (item.id == paramObj.todoID) {
            item.record.push({
                id: Mock.Random.guid(),
                text: paramObj.itemText,
                isDelete: false,
                checked: false
            })
        }
...
//修改小代办的选中状态
Mock.mock(/changeItemChecked/, 'get', (param) => { //三个参数。第一个：路径，第二个：请求方式post/get，第三个：回调，返回值
    console.log(`changeItemChecked,params:`, param)
    let url = param.url
    console.log(url)
    console.log(getParams(url))
    let paramObj = getParams(url);
...
//删除小代办,未完成
Mock.mock(/deleteItem/, 'get', (param) => { //三个参数。第一个：路径，第二个：请求方式post/get，第三个：回调，返回值
    console.log(`deleteItem,params:`, param)
    let url = param.url
    let paramObj = getParams(url);
    data.some(item => {
        if (item.id == paramObj.todoID) {
...
//修改小代办内容
Mock.mock(/changeItemContent/, 'get', (param) => { //三个参数。第一个：路径，第二个：请求方式post/get，第三个：回调，返回值
    console.log(`changeItemContent,params:`, param)
    let url = param.url
    let paramObj = getParams(url);
    data.some(item => {
        if (item.id == paramObj.todoID) {
